...
int enqueueFim(tdeque *f, tdado x){
	...
	if(f->ini==NULL) // testando se esta vazia
	  ...
	  f->fim->prox = novo; //Encadiando
	
	f->fim = novo;    	
	return 1;
}

int enqueueInicio(tdeque *f, tdado x){
	tno *novo = malloc(sizeof(tno));
	if(novo==NULL) // memoria ta cheia
	  return 0;
	novo->dado = x;
...
void printQueueInvertida(tdeque f){ //alterar
	while(f.fim != NULL){
		printf("%d -",f.fim->dado); // mostro o dado
		f.fim = f.fim->ant; // deslocando para o prox
	}// fim while
	printf("\n");
}

int primeiro(tdeque f){
	return f.ini->dado;
}
